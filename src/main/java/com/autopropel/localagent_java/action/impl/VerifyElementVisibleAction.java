...
public class VerifyElementVisibleAction implements ActionHandler {
    @Override
    public void execute(WebDriver driver, TestStep step, ExecutionService context) throws Exception {
        ...
        step.actualValue = visible ? "visible" : "not visible";
        if (visible) {
            step.result_status = 1;
        } else {
            step.result_status = 0;
            throw new AssertionError("VerifyElementVisible FAILED — Element is not visible: " + (step.objectDetail != null ? step.objectDetail : step.data));
        }
    }
}
